...
export async function getComentariiTichet(
  request: Request,
  response: Response
) {
  ...
  if (!idProprietar) {
    return response.status(404).json({ mesaj: "Proprietarul nu a fost găsit" });
  }
  ...
  if (proprietar.rol === "FIRMA") {
    comentariiProprietar = await getComentariiProprietarFirma(id, idProprietar);
  } else {
...
export async function adaugaComentariuNou(
  request: Request,
  response: Response
) {
  ...
  if (request.session.utilizator) {
    ...
    response.status(400).json({ mesaj: "Comentariul nu a putut fi adăugat!" });
  }
}
