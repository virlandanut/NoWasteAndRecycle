...
export default function Inregistrare() {
  ...
  return (
    <SectiuneMain tailwind="flex flex-col justify-center items-center">
      <h1 className="uppercase xl:text-4xl lg:text-3xl md:text-2xl lg:w-full xs:w-3/4 text-center mb-10 text-gray-900 font-bold">
        Doriți să folosiți aplicația în calitate de ...
      </h1>
      <SectiunePaper tailwind="w-full flex xl:flex-row xl:w-[1000px] lg:flex-row lg:w-[800px] md:w-[350px] md:flex-col xs:w-[250px] xs:flex-col">
        <Link
          className={`w-full ${secondSelected ? "grayscale z-0" : "z-10"}`}
          to={"/register/persoana"}
          onMouseEnter={handleHoverFirst}
          onMouseLeave={handleHoverLeave}>
          <div className="flex flex-col items-center justify-center h-full bg-emerald-700 overflow-hidden cursor-pointer border-4 border-solid p-7 md:pb-0 md:pt-3 xs:pt-3 xs:pb-0 border-transparent transition-all duration-500 hover:border-white hover:scale-105 hover:bg-emerald-600 md:hover:border-2 xs:hover:border-2">
